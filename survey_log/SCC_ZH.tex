...
為了說明 software supply chain attacks 的攻擊面並不侷限於單一套件漏洞，本節整理六種具有代表性的攻擊形式。相關研究曾將 SSC 攻擊整理成更完整的 taxonomy，涵蓋名稱偽裝、套件接管、帳號濫用、registry 行為與建置流程等多種攻擊途徑\cite{ssc-taxonomy}。本文僅選取與本簡報主軸最相關的六類，用來銜接後續 Axios 與 TanStack 兩個案例。

\begin{table}[h]
\centering
\caption{代表性的 software supply chain attack forms}
\begin{tabularx}{\textwidth}{P{4cm}Y}
\toprule
\textbf{Attack Form} & \textbf{Description} \\
\midrule
Typosquatting & 攻擊者發布名稱與熱門套件相似的惡意套件，利用拼字錯誤或近似命名誘使開發者誤裝。 \\
Dependency Confusion & 當專案同時使用 public registry 與 private registry 時，攻擊者在 public registry 發布同名套件，使 package manager 解析到錯誤來源。 \\
Account Takeover & 攻擊者竊取 maintainer 或 developer 帳號憑證，接著使用合法帳號權限發布惡意套件版本。 \\
Package Takeover & 攻擊者取得無人維護、轉移所有權或信任邊界鬆散的套件控制權，再發布帶有惡意內容的新版本。 \\
Code Injection & 攻擊者將惡意程式碼注入到原本受信任的 package、dependency 或 release artifact 中，使惡意行為混入正常軟體流程。 \\
CI/CD Compromise & 攻擊者濫用 build workflows、cache、runner environment 或 tokens，使合法的建置與發布流程產出惡意 artifact。 \\
\bottomrule
\end{tabularx}
\end{table}

這些攻擊形式說明，software supply chain security 不只是在檢查 source code 本身，也必須涵蓋套件名稱、maintainer accounts、dependency resolution、package registries、build pipelines 與 release process。後續 Axios 事件主要展示 account takeover、malicious package publication 與 installation script abuse 的風險；TanStack 事件則進一步展示 CI/CD compromise 與 cache trust boundary 的問題。
...
實務上，build provenance 涉及多個技術層次。SLSA 定義 provenance 與 build levels 所需證明的內容；Sigstore 提供 keyless signing 與 Rekor transparency log；registry 與 CI platform 負責產生、儲存與發布 attestations；consumer-side tools 如 npq 或 Socket Firewall 則可作為安裝前的檢查機制，用於 provenance verification，並輔助判斷套件安裝風險。
...
Consumer & npq / Socket Firewall & 作為 pre-install gate，用於 provenance verification，並輔助判斷套件安裝風險。 \\
\bottomrule
\end{tabularx}
\end{table}
