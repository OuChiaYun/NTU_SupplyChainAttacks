...
The two case studies show that no single defense mechanism can cover the entire supply chain attack surface: stolen credentials, lifecycle script abuse, CI/CD cache poisoning, and OIDC token theft each require different controls. Practical defense must therefore be layered.

\subsection{Dependency Layer}

The goal of the dependency layer is to reduce the probability that malicious or vulnerable packages enter a project. In practice, organizations should use lockfiles to control versions and hashes, and they should avoid unnecessary dependency updates. SBOMs can record dependency composition, while vulnerability scanners such as \texttt{npm audit} and OSV \cite{osv} can detect known vulnerabilities. However, these tools are insufficient for newly published malicious packages. Therefore, install-time gates such as npq \cite{npq} or Socket Firewall \cite{socket-firewall} should also be used to check package metadata, provenance status, and suspicious behavior. Static behavior analysis tools such as GuardDog \cite{guarddog} can further scan package source code for suspicious patterns, providing a detection layer that does not depend on CVE records. In high-risk environments, a minimum release age policy can reduce the probability that a newly published malicious version enters a project immediately \cite{openai-tanstack}.
...
To ground the theoretical analysis in an observable demonstration, we conducted a controlled experiment at the dependency layer. The central question: if a malicious npm package has no CVE and appears in no vulnerability database, but executes clearly suspicious behavior at install time, can common defensive tools detect it?

We built a simulated malicious package, \texttt{demo-malicious-tool} \cite{demo-pkg,experiment-repo}, modelled on the Axios-style install-time attack. Its \texttt{postinstall} script uses \texttt{INIT\_CWD} to locate the installing project, reads its \texttt{.env}, serializes \texttt{process.env}, and sends a JSON payload via HTTP POST to a local test listener. A demo project depending on this package was then scanned with four tools spanning three defense categories: npm audit and OSV \cite{osv} (known-vulnerability matching), npq \cite{npq} (supply chain metadata signals), and GuardDog \cite{guarddog} (static behavior analysis). A real package, \texttt{is-number@7.0.0} \cite{isnumber} --- feature-stable since 2018 and without provenance --- was included as a benign counterpart to observe false-positive behavior.

\begin{table}[h]
\centering
\caption{Expected and actual scan results for \texttt{demo-malicious-tool}}
\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{P{2.2cm}P{4.2cm}P{2.8cm}Y}
\toprule
\textbf{Tool} & \textbf{Category} & \textbf{Expected} & \textbf{Actual} \\
\midrule
npm audit & CVE / advisory lookup & Not flagged & Not flagged \\
\addlinespace
OSV & Vulnerability database & Not flagged & Not flagged \\
\addlinespace
npq & Metadata signals & Inconclusive & Not found (GitHub dependency) \\
\addlinespace
GuardDog & Static behavior analysis & Flagged & Flagged (3 rules) \\
\bottomrule
\end{tabularx}
\renewcommand{\arraystretch}{1}
\end{table}

The results match expectations. npm audit and OSV did not flag the package because no advisory record exists for a newly created package --- this is a design boundary, not a failure: these tools answer whether a known vulnerability has been recorded, not whether the code behaves maliciously. npq could not evaluate the package because it is a GitHub dependency rather than a registry package, exposing a coverage gap for non-registry sources. GuardDog flagged three behavioral rules --- \texttt{npm-serialize-environment}, \texttt{npm-exfiltrate-sensitive-data}, and \texttt{npm-install-script} --- detecting the attack pattern from source code without any prior record.

\texttt{is-number@7.0.0} was flagged by both npq (expired domain, missing provenance, package age) and GuardDog (one rule). As a widely used and benign package, these are false positives, confirming that metadata signals and behavioral rules require human interpretation rather than automated rejection.

One important caveat applies to the experiment as a whole: \texttt{demo-malicious-tool} is a GitHub dependency with no registry presence or download history. In practice, developers would have reason to question installing such an unknown package; real supply chain attacks are more subtle, exploiting compromised existing packages or naming proximity rather than unknown repositories. The experiment therefore demonstrates the detection gap under controlled conditions. Its finding --- that behavioral analysis can detect malicious install-time patterns independently of vulnerability databases --- should be read as a proof of concept, not a simulation of a complete real-world attack.

\section{Conclusion}

Software supply chain attacks succeed because they exploit trust across every layer of modern development: package managers, maintainer accounts, registries, build workflows, and release pipelines. Attackers need not breach an application directly --- a single compromised upstream node propagates malicious code through normal installation.

The two case studies and the experimental validation together define the boundary of provenance as a control. Axios shows that a provenance regression check would have been decisive against a stolen-credential attack. TanStack shows that provenance is insufficient when the build environment is subverted from within. The experiment further shows that CVE-based scanning misses newly published malicious packages, while behavioral analysis can detect suspicious patterns without any prior record. Across all three cases, no single mechanism covers the full attack surface.

Build provenance is therefore necessary but not sufficient. Effective defense requires it as one layer among many --- behavioral analysis, dependency scanning, lockfile discipline, minimum release age policies, maintainer account protection, CI/CD isolation, and credential restriction --- each answering a different question, together reducing a systemic risk that no individual control can eliminate alone.
...
\bibitem{axios-npm-stats}
npm.
\textit{axios: Weekly Downloads}.
...
\bibitem{demo-pkg}
OU, Chia-Yun (2026).
\textit{demo-malicious-tool: Simulated malicious npm package for supply chain experiment}.
\url{https://github.com/OuChiaYun/demo-malicious-tool}

\bibitem{experiment-repo}
OU, Chia-Yun; NG, Chun Sing (2026).
\textit{NTU\_SupplyChainAttacks: npm Supply Chain Risk Gate}.
\url{https://github.com/OuChiaYun/NTU_SupplyChainAttacks}

\bibitem{isnumber}
Jon Schlinkert.
\textit{is-number (npm package)}.
\url{https://www.npmjs.com/package/is-number}
Accessed: June 3, 2026.

\bibitem{osv}
Google (2021).
\textit{OSV: Open Source Vulnerabilities}.
\url{https://osv.dev/}
Accessed: June 3, 2026.

\bibitem{guarddog}
Datadog (2022).
\textit{GuardDog: Identify malicious PyPI and npm packages}.
